...
function LoginFormFunctional() {
  ...
  const handleChange = (event) => {
    ...
    setLoginData((prevState) => {
      return {
        ...prevState,
        [name]: value,
        is_login_button_disable: true,
      };
    });
    if (
      loginData.errors.email.trim() === "" &&
      loginData.errors.password.trim() === ""
    ) {
      setLoginData((prevState) => {
        return {
          ...prevState,
  ...
  return (
    <form className={`${style.login_form_content}`} onSubmit={handleOnLogin}>
      <div>
        <label>Email</label>
        <input
          type="text"
      ...
      <div className={style.login_button}>
        <CustomButton
          name={"Login"}
          isDisable={loginData.is_login_button_disable}
          type={"submit"}
        />
      </div>
